...
class CommonOptionsValidator(BaseOptionsValidator):
    ...
    def _check_genetic_variables(self) -> bool:
        ...
        for v in self.options._genetic_variables:
            if v.min_value is None or v.max_value is None or v.step is None:
                self._add_message("Min value, max value and step must not be None! (Variable {var_no})".format(var_no = v.variable_id))
                rslt &= False
                ...
                self._add_message("Min value, max value and step must be float or int values! (Variable {var_no})".format(var_no = v.variable_id))
                rslt &= False
            elif v.min_value > v.max_value:
                self._add_message("Min value must be less or equal than max value! (Variable {var_no})".format(var_no = v.variable_id))
                rslt &= False
            elif v.step <= 0.0:
                self._add_message("Step must be positive float value! (Variable {var_no})".format(var_no = v.variable_id))
                rslt &= False
            elif v.min_value + v.step > v.max_value:
                self._add_message("Invalid step value! (Variable {var_no})".format(var_no = v.variable_id))
                rslt &= False
            if v.decimal_places < 0:
                self._add_message("Invalid number of decimal places! (Variable {var_no})".format(var_no = v.variable_id))
                rslt &= False
        return rslt
    ...
    def _check_cost_function(self):
        ...
        try:
            ...
            self._add_message("Inadequate number of parameters for the passed function!")
            return False
        except Exception as ex:
            self._add_message("An exception raised on the function call: {exc}".format(exc=str(ex)))
            return False
        return True
    ...
    def _check_nones_types_and_values(self) -> bool:
        ...
        if self.options.immigration_number is None:
            ...
            self._add_message("Immigration Number must not be type same or greater than population size!")
            rslt &= False        
        ...
        if self.options.number_of_mutation_chromosomes is None and self.options.percentage_of_mutation_chromosomes is None:
            self._add_message("Number of Mutation Chromosomes or Percentage of Mutation Chromosomes must not be None!")
            rslt &= False
            ...
            self._add_message("Number of Mutation Chromosomes must be type int or Percentage of Mutation Chromosomes must be float!")
            rslt &= False
        else:
            if (not self.options.number_of_mutation_chromosomes is None) and (isinstance(self.options.number_of_mutation_chromosomes, int)) and (self.options.number_of_mutation_chromosomes > 0):
                if population_size > 0 and self.options.number_of_mutation_chromosomes > (population_size // 2) - immigration_number:
                    self._add_message("Invalid number of mutation chromosomes: {0}".format(str(self.options.number_of_mutation_chromosomes)))
                    rslt &= False
                ...
                self._add_message("Number of Mutation Chromosomes must have int value >=0 or Percentage of Mutation Chromosomes must have float (0.0-100.0) value! ")
                rslt &= False
            elif self.options.percentage_of_mutation_chromosomes < 0.0 or self.options.percentage_of_mutation_chromosomes > 100.0:
                self._add_message("Invalid percentage of mutation chromosomes: {0} and number of mutation chromosomes: {1}".format(str(self.options.percentage_of_mutation_chromosomes, self.options.number_of_mutation_chromosomes)))
                rslt &= False
        if self.options._genetic_variables is None:
        ...
        if self.options.number_of_mutation_genes is None and self.options.percentage_of_mutation_genes is None:
            self._add_message("Number Of Mutation Genes or Percentage Of Mutation Genes must not be None!")
            rslt &= False
            ...
            self._add_message("Number Of Mutation Genes must have int value >=0 or Percentage Of Mutation Genes must have float (0.0-100.0) value!")
            rslt &= False        
            ...
            if (number_of_genetic_variables > 0 and self.options.number_of_mutation_genes > number_of_genetic_variables):
                self._add_message("Invalid number of mutation genes: {0}".format(
            ...
            self._add_message("Number of Mutation Genes must have int value or Percentage of Mutation Genes must have float value! ")
            rslt &= False
        elif self.options.percentage_of_mutation_genes < 0.0 or self.options.percentage_of_mutation_genes > 100.0:
            self._add_message("Invalid percentage of mutation genes: {0} and number of mutation genes: {1}".format(str(self.options.percentage_of_mutation_genes, self.options.number_of_mutation_genes)))
            rslt &= False
        ...
        if self.options.chromosome_mutation is None:
            ...
            self._add_message("Invalid value of Chromosome Mutation: {0}. Allowed values: {1}".format(self.options.chromosome_mutation, self._get_allowed_values(definitions.CHROMOSOME_MUTATOR_STRINGS)))
            rslt &= False   
        if self.options.exit_check is None:
            ...
            self._add_message("Invalid value of Exit Check: {0}. Allowed values: {1}".format(self.options.exit_check, self._get_allowed_values(definitions.EXIT_CRITERIA_STRINGS)))
            rslt &= False
        if self.options.chromosome_mutation == definitions.CROSS_DIVERSITY:
            if self.options.cross_diversity_mutation_gene_selection is None:
                self._add_message("Cross Diversity Mutation Gene Selection must not be None!")
                rslt &= False
            elif not isinstance(self.options.cross_diversity_mutation_gene_selection, str):
                self._add_message("Cross Diversity Mutation Gene Selection must be type str!")
                rslt &= False
            elif not self._validate_selection(self.options.cross_diversity_mutation_gene_selection, "Cross Diversity Mutation Gene Selection", self.options.number_of_mutation_genes, "Group Size for Cross Diversity Mutation Gene Selection must be below or equal than the number of mutation genes!"):
                rslt &= False           
        ...
        if self.options.population_mutation == definitions.COST_DIVERSITY or definitions.PARENT_DIVERSITY in self.options.population_mutation:
            if self.options.parent_diversity_mutation_chromosome_selection is None:
                self._add_message("Parent Diversity Mutation Chromosome Selection must not be None!")
                rslt &= False
            elif not isinstance(self.options.parent_diversity_mutation_chromosome_selection, str):
                self._add_message("Parent Diversity Mutation Chromosome Selection must be type str!")
                rslt &= False
            elif not self._validate_selection(self.options.parent_diversity_mutation_chromosome_selection, "Parents Diversity Mutation Chromosome Selection", (population_size // 2) - self.options.immigration_number, "Group Size for Parents Diversity Mutation Chromosome Selection cannot have the value below half population!"):
                rslt &= False 
        if self.options.population_mutation is None:
            ...
            for mutator_string in mutator_strings:
                if mutator_string not in definitions.POPULATION_MUTATOR_STRINGS:
                    self._add_message("Invalid value of Population Mutation: {0}. Allowed values: {1}".format(mutator_string, self._get_allowed_values(definitions.POPULATION_MUTATOR_STRINGS)))
                    rslt &= False
        elif self.options.population_mutation not in definitions.POPULATION_MUTATOR_STRINGS:
            self._add_message("Invalid value of Population Mutation: {0}. Allowed values: {1}".format(self.options.population_mutation, self._get_allowed_values(definitions.POPULATION_MUTATOR_STRINGS)))
            rslt &= False
        if self.options.parent_selection is None:
            ...
            rslt &= False
        elif not self._validate_selection(self.options.parent_selection, "Parent Selection", (population_size // 2) - self.options.immigration_number, "Group Size for parent selection cannot have the value below half population!"):
            rslt &= False       
        ...
        if self.options.timeout is None:
            ...
            self._add_message("Invalid timeout value: {0}".format(str(self.options.timeout)))
            rslt &= False
        return rslt
    
    def _validate_selection(self, selection_string, selection_type, comparing_number, group_size_error_message) -> bool:
        ...
        if len(selection_strings) > 2 or (len(selection_strings) > 1 and selection_strings[0] != definitions.TOURNAMENT):
            self._add_message("Invalid {0} value: {1}".format(selection_type, selection_string))
            return False
        if selection_strings[0] == definitions.TOURNAMENT and len(selection_strings) > 1:
            ...
            try:
                ...
                return False
            if n_group_size < 2:
                self._add_message("Invalid {0} value: {1}. Minimal group size is 2.".format(selection_type, selection_string))
                return False
            if not isinstance(n_group_size, int):
                self._add_message("Invalid value for {0}: {1}".format(selection_type, selection_string))
                return False
        ...
        if not selection_strings[0] in definitions.SELECTION_STRINGS:
            self._add_message("Invalid {0} value: {1}. Allowed values: {2}".format(selection_type, selection_string, self._get_allowed_values(definitions.SELECTION_STRINGS)))
            return False
        return True
