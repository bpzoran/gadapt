...
class NormalDistributionCrossDiversityGeneMutator(NormalDistributionGeneMutator):
    ...
    def _calculate_normal_distribution_standard_deviation(self):
        if self.gene_value.gene.columnar_diversity_coefficient is None or math.isnan(
            self.gene_value.gene.columnar_diversity_coefficient
        ):
        ...
        dv_rsd = np.clip(self.gene_value.gene.columnar_diversity_coefficient, 0, 1)
